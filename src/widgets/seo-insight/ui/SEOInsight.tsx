...
const iconColorMap: Record<string, { bg: string; text: string; check: string }> = {
  'server': { bg: 'bg-blue-500/10', text: 'text-blue-400', check: 'text-blue-400' },
  'settings': { bg: 'bg-accent-primary/10', text: 'text-accent-primary', check: 'text-accent-primary' },
  'zap': { bg: 'bg-green-500/10', text: 'text-green-400', check: 'text-green-400' },
}
            ...
            {cards.map((card, idx) => {
              ...
              if (card.isHighlighted) {
                return (
                  <div
                    key={idx}
                    className="p-8 rounded-3xl bg-gradient-to-br from-bg-elevated to-bg-secondary border border-accent-primary shadow-[0_0_30px_rgba(79,70,229,0.15)] relative overflow-hidden group"
                  >
              ...
              return (
                <div key={idx} className="p-8 rounded-3xl bg-bg-secondary border border-white/5">
                  <div className={`w-12 h-12 rounded-xl ${colors.bg} flex items-center justify-center mb-6`}>
                    <Icon className={`w-6 h-6 ${colors.text}`} />
                  </div>
                  <h3 className="text-xl font-bold mb-4">{card.title}</h3>
