...
import { Button, Container, SectionHeading, AnimatedSection } from '@/shared/ui'
            ...
            {plans.map((tier, i) => (
              <div
                ...
                className={cn(
                  ...
                  tier.isPopular
                    ? 'bg-gradient-to-b from-bg-elevated to-bg-secondary border border-accent-primary shadow-[0_0_40px_rgba(79,70,229,0.15)] transform md:-translate-y-4'
                    : 'bg-bg-secondary border border-white/5'
                )}
              >
                {tier.isPopular && (
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-gradient-to-r from-accent-primary to-accent-secondary text-white text-xs font-bold px-4 py-1 rounded-full uppercase tracking-wider">
                    Популярный выбор
                  </div>
                )}
